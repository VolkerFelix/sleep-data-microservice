...
class SleepPhases(BaseModel):
    ...
    awake_minutes: Optional[int] = Field(0, description="Minutes spent awake during sleep period")
...
class BreathingData(BaseModel):
    ...
    average_rate: Optional[float] = Field(None, description="Average breathing rate (breaths per minute)")
    disruptions: Optional[int] = Field(None, description="Number of breathing disruptions/irregularities")

class SleepMetadata(BaseModel):
    """Model for sleep data metadata."""
    source: str = Field(..., description="Source of the sleep data (e.g., 'apple_health', 'generated')")
    generated_at: Optional[datetime] = Field(None, description="When the data was generated (for synthetic data)")
    imported_at: Optional[datetime] = Field(None, description="When the data was imported")
    device: Optional[str] = Field(None, description="Device that recorded the data")
    source_name: Optional[str] = Field(None, description="Name of the source application")
    version: Optional[str] = Field(None, description="Version of the source application")
    raw_data: Optional[Dict[str, Any]] = Field(None, description="Raw data from the source")
...
class SleepTimeSeries(BaseModel):
    ...
    respiration_rate: Optional[float] = Field(None, description="Respiration rate at this time")
...
class SleepRecord(BaseModel):
    ...
    sleep_phases: Optional[SleepPhases] = Field(None, description="Breakdown of sleep phases")
    sleep_quality: Optional[int] = Field(None, description="Sleep quality score (0-100)")
    heart_rate: Optional[HeartRateData] = Field(None, description="Heart rate data during sleep")
    breathing: Optional[BreathingData] = Field(None, description="Breathing data during sleep")
    environment: Optional[SleepEnvironment] = Field(None, description="Sleep environment data")
    time_series: Optional[List[SleepTimeSeries]] = Field(None, description="Time series data for the sleep session")
    tags: Optional[List[str]] = Field(None, description="Tags associated with this sleep record")
    notes: Optional[str] = Field(None, description="Notes about this sleep record")
    metadata: SleepMetadata = Field(..., description="Metadata about the sleep record")
...
class SleepRecordUpdate(BaseModel):
    ...
    date: Optional[str] = Field(None, description="Date of the sleep record (YYYY-MM-DD)")
    ...
    duration_minutes: Optional[int] = Field(None, description="Total duration of sleep in minutes")
    sleep_phases: Optional[SleepPhases] = Field(None, description="Breakdown of sleep phases")
    sleep_quality: Optional[int] = Field(None, description="Sleep quality score (0-100)")
    heart_rate: Optional[HeartRateData] = Field(None, description="Heart rate data during sleep")
    breathing: Optional[BreathingData] = Field(None, description="Breathing data during sleep")
...
class GenerateSleepDataRequest(BaseModel):
    ...
    user_id: str = Field(..., description="User ID to associate with the generated data")
    ...
    include_time_series: bool = Field(False, description="Whether to include detailed time series data")
    sleep_quality_trend: Optional[str] = Field(None, description="Trend for sleep quality: 'improving', 'declining', 'stable', 'random'")
    sleep_duration_trend: Optional[str] = Field(None, description="Trend for sleep duration: 'increasing', 'decreasing', 'stable', 'random'")

class SleepStats(BaseModel):
    """Model for sleep statistics."""
    average_duration_minutes: float = Field(..., description="Average sleep duration in minutes")
    average_sleep_quality: Optional[float] = Field(None, description="Average sleep quality score")
    average_deep_sleep_minutes: Optional[float] = Field(None, description="Average deep sleep duration in minutes")
    average_rem_sleep_minutes: Optional[float] = Field(None, description="Average REM sleep duration in minutes")
    average_light_sleep_minutes: Optional[float] = Field(None, description="Average light sleep duration in minutes")
...
class SleepAnalyticsResponse(BaseModel):
    ...
    recommendations: Optional[List[str]] = Field(None, description="Sleep recommendations based on the analysis")
...
class AppleHealthImportRequest(BaseModel):
    ...
    data_type: Optional[str] = Field("export_xml", description="Type of Apple Health data: 'export_xml' or 'direct'")
