...
class SleepDataService:
        ...
        while current_date <= end_date:
            ...
            if sleep_quality_trend:
                if sleep_quality_trend == "improving":
                    quality_modifier = progress_factor * 15  # Improve by up to 15 points
                elif sleep_quality_trend == "declining":
                    quality_modifier = -progress_factor * 15  # Decline by up to 15 points
            ...
            if sleep_duration_trend:
                if sleep_duration_trend == "increasing":
                    ...
                    duration_modifier = -progress_factor * 2  # Decrease by up to 2 hours
            ...
            sleep_duration_hours = max(4.0, min(10.0, base_duration + duration_modifier + random.uniform(-0.5, 0.5)))
            ...
            light_sleep_minutes = int(total_minutes - deep_sleep_minutes - rem_sleep_minutes)
            ...
            min_heart_rate = avg_heart_rate - (5 + (quality_factor * 10)) + random.uniform(-2, 2)
            max_heart_rate = avg_heart_rate + (15 - (quality_factor * 5)) + random.uniform(-2, 2)
            ...
            if include_time_series:
                ...
                # Determine sleep stage transitions based on normal sleep cycle patterns
                ...
                while current_time < sleep_end:
                    ...
                    minutes_into_sleep = (current_time - sleep_start).total_seconds() / 60
                    cycle_position = (minutes_into_sleep % cycle_minutes) / cycle_minutes
    ...
    def analyze_sleep_data(
        ...
        quality_scores = [record.get("sleep_quality") for record in sleep_records if record.get("sleep_quality") is not None]
        ...
        avg_deep = sum(deep_sleep_mins) / len(deep_sleep_mins) if deep_sleep_mins else None
        avg_rem = sum(rem_sleep_mins) / len(rem_sleep_mins) if rem_sleep_mins else None
        avg_light = sum(light_sleep_mins) / len(light_sleep_mins) if light_sleep_mins else None
        ...
        recommendations = self.generate_sleep_recommendations(
        ...
        return {
            ...
            "stats": {
                ...
                "average_light_sleep_minutes": round(avg_light, 1) if avg_light else None,
    ...
    def calculate_sleep_trends(self, sleep_records: List[Dict]) -> Dict[str, Any]:
        ...
        start_times = [datetime.fromisoformat(record["sleep_start"]).time() for record in sorted_records]
        start_time_minutes = [(t.hour * 60 + t.minute) % (24 * 60) for t in start_times]  # Convert to minutes past midnight
        schedule_consistency = self._calculate_consistency(start_time_minutes)
        
        # Day-to-day variability in duration
        if len(durations) > 1:
            duration_differences = [abs(durations[i] - durations[i+1]) for i in range(len(durations)-1)]
            avg_duration_difference = sum(duration_differences) / len(duration_differences)
            duration_variability = avg_duration_difference / (sum(durations) / len(durations))  # Normalized
        else:
            duration_variability = 0
        
        return {
            "duration_trend": {
                "direction": "increasing" if duration_trend > 0.01 else "decreasing" if duration_trend < -0.01 else "stable",
                "strength": abs(duration_trend),
                "average_change_per_day": duration_trend * 60  # Convert back to minutes
            },
            "quality_trend": {
            ...
            } if quality_trend is not None else None,
            "schedule_consistency": {
                "score": 100 - min(100, schedule_consistency),  # Convert to a 0-100 score where 100 is perfectly consistent
                "rating": "excellent" if schedule_consistency < 30 else
                         "good" if schedule_consistency < 60 else
                         "fair" if schedule_consistency < 90 else
                         "poor"
            },
            "duration_variability": {
                "score": 100 - min(100, duration_variability * 100),
                "rating": "excellent" if duration_variability < 0.1 else
                         "good" if duration_variability < 0.2 else
                         "fair" if duration_variability < 0.3 else
                         "poor"
            }
        }
    ...
    def _calculate_trend(self, values: List[float]) -> float:
        ...
        Returns:
            float: average change per step (positive for increasing, negative for decreasing)
        """
        if len(values) < 2:
            return 0
    ...
    def _calculate_consistency(self, time_minutes: List[int]) -> float:
        ...
        variance = sum((t - adjusted_mean) ** 2 for t in adjusted_times) / len(adjusted_times)
